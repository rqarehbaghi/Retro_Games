...
| `UPLOAD_BRIEF.md` | paste into Codex and it walks the upload -- the copy, the rules, and Windows-side paths for the files to attach |
...
- `writer.py` — every prompt and all four LLM backends (`Codex`, `Codex`, `gemini`, `ollama`) plus the `auto` cascade
...
`--writer` in `studio.json`. **A Codex Pro subscription and the Messages API
...
answers: Codex (subscription) → Codex API (credits) → Gemini
...
| `Codex` | the Pro/Max **subscription**, via `Codex -p` | no — asked for in the prompt |
| `Codex` | prepaid API **credits** | yes — API structured output |
...
Codex's own login. If auth looks wrong, check `env | grep -i anthropic`
first.
